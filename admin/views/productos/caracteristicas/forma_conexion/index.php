<div class="ps-5 pe-5 pt-3 my-container active-cont">
    ...
    <table class="table">
        ...
        <?php foreach($datos as $key => $forma_conexion): ?>
            <tr>
                ...
                <td>
                    <a href="formas_conexion.php?action=ver&id_forma_conexion=<?=$forma_conexion['id_forma_conexion']?>" class="btn btn-outline-primary">
                        <i class="fa fa-arrow-up p-1 icons"></i>
                    </a>
                    <a href="formas_conexion.php?action=eliminar&id_forma_conexion=<?=$forma_conexion['id_forma_conexion']?>" class="btn btn-outline-danger">
    ...
    <nav>
        <ul class="pagination">
            <?php for($i = 0, $k = 1; $i < $formas_conexion -> total(); $i+=3, $k++): ?>
                <li class="page-item"><a class="page-link" href="formas_conexion.php?<?php echo(isset($_GET['busqueda']))?'busqueda='.$_GET['busqueda'].'&':''; ?>&desde=<?php echo($i); ?>&limite=3"><?php echo ($k); ?></a></li>
            <?php endfor; ?>
        </ul>
    </nav>
    <?php
